 ...
 *   Tiempo de muestreo 100ms  ---> Ts = 10
 *   Cuenta de muescas        ---> n
 *
 ...
 *
 *
 *    Esta expresion implica que la medida detectable es
 *   	375RPM
 *****************************************************/
#include <stdlib.h>
#include <stdint.h>
...
volatile unsigned char OvCount2= 0;
...
ISR(TIMER0_OVF_vect)
{
	OvCount++;
	//ticks++;
	if (OvCount == 4880)  //49 = 100ms aprox
	{
		flagSample=1;
		OvCount =0;	//OCR0A = 122;
...
int main()
{
  ...
  float error = 0.0;
	int referencia = 250; //velocidad tick en 106ms
	int u = 0;
	int u_k = 40;
	float Kp = 2.00;//Kp = 255/35 a max vel cuenta 35 pulsos en 106ms
	AVRInit();
  // Infinite loop
	while(1)
	{
